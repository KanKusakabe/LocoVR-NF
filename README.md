...
**▶ 多層拡張(2026-07-13)**: この2フロー(環境層 `p(居場所｜占有)` × 行動層 `p(次歩｜goal)`)を**縦に積む**トラックを追加。[MULTILAYER.md](MULTILAYER.md) / [multilayer.html](multilayer.html) 参照。B(積フィルタ)=未知レイアウトで後付け環境層が家具重なり23%減、粗→細(階層)=C3で失敗した大域経路識別を粗フロー尤度が回復(実経路上位 16%→82%)。
...
`index.html` のデモは**本物の学習済み重み**(C3 StepFlow・C1 AffordanceFlow を JS 移植、PyTorch と誤差 1e-6/9e-6 で照合)で動くが、静的ページゆえの割り切りがある。**主張・定量はローカル(Python)側が本体**。

- **① フローを ONNX/TF.js でそのまま出せない**(自己回帰RQスプライン流の逆写像)→ CropCNN+NSF を手で JS 移植し数値照合。
- **② モデル単体はほぼ操舵しない(最本質)**:学習ステップは前進バイアスが強く(fwd≈0.29 / 横 lat≈±0.07)、大域的な曲がりを学んでいない。→ 純粋なモデル任せロールアウトはゴール未到達。外側の**目的地コントローラ**(ゴール方向へ向け+家具を迂回)が必須で、デモは「モデル=局所の歩き方 / コントローラ=大域の向き」のハイブリッド。
- **③ ブラウザ推論が ~10倍遅い**(CNN 22ms vs 2ms)+ 背景タブで rAF が停止 → `setTimeout` ストリーミング、ドラッグ中は軽量代理。
- **④ 最適化目的のゲーム化**:モデルは「人は壁際に居る」と学習 → 密度の素朴最大化は「家具で囲んで水増し」に退化 → 空室 traffic 場を固定コストに使い回避。
- **⑤ デモの最適化は位置のみ(軸並行)**。`optimize.py` は回転込み。**⑥ 部屋・家具は合成矩形**(モデルと traffic 場は本物、間取りは HM3D 実スキャンでない)。

教訓: **素朴な予測モデルは大域の逆設計に使えない**(Layout-NF A1/A2 の再現)——「学べたこと」と「まだ学べていない大域操舵」の両方がデモに正直に表れている。
